					  ...
					  struct ext2_super_block *super)
{
	...
	printf("write_primary_superblock: partition-%d, first-%d, last-%d, next-%d\n", fs->device_mask, super->s_first_data_block, super->s_blocks_count-1, super->next_sb_blk/4);
	
	if (!fs->io->manager->write_byte || !fs->orig_super) {
	fallback:
		io_channel_set_blksize(fs->io, SUPERBLOCK_OFFSET);
		// dulian: write sb in first data block, no padding
		retval = io_channel_write_blk64(fs->io, super->s_first_data_block * 4, -SUPERBLOCK_SIZE,
					      super);
