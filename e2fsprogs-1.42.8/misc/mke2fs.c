...
{
	fprintf(stderr, _("Usage: %s [-p number-of-partitions]\n"
	"\t[-c|-l filename] [-b block-size] "
...
//modified by eext4
static void create_lost_and_found(ext2_filsys fs, const char *name)
{
...
try_again:
	...
	if(retval == EXT2_ET_DIR_NO_SPACE){
		retval = ext2fs_expand_dir(fs, EXT2_ROOT_INO);
		if(retval){
			com_err("ext2fs_mkdir", retval, _("while expanding directory"));
			return;
		}
		goto try_again;
	}
...
static void PRS(int argc, char *argv[])
{
	...
	char *		fs_type = 0;
	char *		usage_types = 0;
	// dulian: change into global variable
	//blk64_t		dev_size;
...
profile_error:
	...
	while ((c = getopt (argc, argv,
		    "p:b:cg:i:jl:m:no:qr:s:t:vC:DE:FG:I:J:KL:M:N:O:R:ST:U:V")) != EOF) {
		switch (c) {
		case 'p':
			n_partition = strtoul(optarg, &tmp, 0);
			if (*tmp) {
				com_err(program_name, 0,
					_("invalid partition number - %s"), optarg);
				exit(1);
			}
			printf("spanfs partition number: %d\n", n_partition);
			break;
		case 'b':
	...
	// dulian: in PRS() it is regarded as the partition size, overwrite in orig_main()
	ext2fs_blocks_count_set(&fs_param, fs_blocks_count / n_partition);
...
int orig_main (int argc, char *argv[], int partition_no, blk64_t first_data_blk,
	blk64_t last_data_blk, blk64_t next_sb_blk)
{
	errcode_t	retval = 0;
	ext2_filsys	fs;
	...
	// dulian: overwrite some sb member
	fs_param.s_first_data_block = first_data_blk;
	ext2fs_blocks_count_set(&fs_param, last_data_blk + 1);
	fs_param.next_sb_blk = next_sb_blk  * 4; //TODO hard-coded
...
#define EXT2_DFL_CHECKINTERVAL (86400L * 180L)
	...
	if (super_only) {
		...
		for(index=0; index<128; index++){
			sprintf(buffer, "SPANDir-%d", index);
			create_lost_and_found(fs, buffer);
		}
		create_lost_and_found(fs, "EEXT4-ROOT");
...
int main (int argc, char *argv[]) {
	int partition_no;
	blk64_t first_data_blk;
	blk64_t last_data_blk;
	blk64_t next_sb_blk;
	blk64_t partition_size;

	PRS(argc, argv); // to get dev_size and n_partition
	
	partition_size = dev_size / n_partition / 4;
	
	for(partition_no=n_partition-1; partition_no>=0; partition_no--) {
		first_data_blk = partition_size * partition_no;
		last_data_blk = first_data_blk + partition_size - 1;
		next_sb_blk = (partition_no == n_partition - 1) ? 0 : last_data_blk + 1;
		printf("main: partition_no-%d, first_data_blk-%llu, last_data_blk-%llu, next_sb_blk-%llu\n", partition_no, first_data_blk, last_data_blk, next_sb_blk);
		orig_main(argc, argv, partition_no, first_data_blk, last_data_blk, next_sb_blk);
		printf("\n\n");
	}
